# -*- coding: utf-8 -*-
"""Define a class to use basic lstm to predict the stock index."""
...
class LSTM(BasicModel):
    """use the standard LSTM to do the time series prediction."""
    ...
    def inference(self):
        ...
        #     input, [-1, self.num_steps, para.NUM_INPUT])
        ...
        self.rnn_output = tf.reshape(
            tf.concat(outputs, 1), [-1, self.hidden_size])
        # Final scores and predictions
        with tf.name_scope("output"):
            W = tf.get_variable(
                "W",
                shape=[self.hidden_size, para.NUM_OUTPUT],
